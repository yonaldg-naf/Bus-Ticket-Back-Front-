    ...
    <section class="container max-w-6xl pt-10">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-muted">Add, edit and manage your fleet.</p>
        </div>
          ...
          <div class="card-body">
            ...
            <form [formGroup]="form" (ngSubmit)="onSubmit()" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input formControlName="code" type="text" placeholder="e.g. BUS-001" class="input"/>
                @if (isInvalid('code')) {
                  <p class="text-xs text-red-600 mt-1">Code is required (max 50 chars)</p>
                }
              </div>

              <div>
                <label class="label">Registration No *</label>
                <input formControlName="registrationNumber" type="text" placeholder="e.g. MH12AB1234" class="input"/>
                @if (isInvalid('registrationNumber')) {
                  <p class="text-xs text-red-600 mt-1">Registration is required (max 50 chars)</p>
                }
              </div>

              <div>
                <label class="label">Bus Type *</label>
                <select formControlName="busType" class="input bg-white">
                  <!-- keep numeric values to match your existing DTOs -->
              ...
              <div>
                ...
                <input formControlName="totalSeats" type="number" min="1" max="100" placeholder="40" class="input"/>
                @if (isInvalid('totalSeats')) {
                  <p class="text-xs text-red-600 mt-1">Seats must be between 1 and 100</p>
                }
              </div>

              <div>
                <label class="label">Status</label>
                <select formControlName="status" class="input bg-white">
      ...
      <div class="space-y-3">
            ...
            <div class="card-body">
              <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div class="min-w-0">
                  ...
                  <p class="text-sm text-muted mt-1">Seats: {{ bus.totalSeats }}</p>
                </div>

                <!-- Right actions -->
                <div class="flex items-center gap-2">
                  <button class="btn btn-secondary h-9" (click)="editBus(bus)">Edit</button>
                  <button class="btn btn-ghost h-9 border border-red-200 text-red-600 hover:bg-red-50"
        ...
        @if (!loading() && buses().length === 0) {
          <div class="text-center py-16">
            ...
            <p class="text-muted mt-1">Click “Add Bus” to create your first bus.</p>
          </div>
        }
      </div>
    </section>
...
export class ManageBusesComponent implements OnInit {
  ...
  busStatusLabel(s: number): string {
    const map: Record<number, string> = { 1: 'Available', 2: 'Under Repair', 3: 'Not Available' };
