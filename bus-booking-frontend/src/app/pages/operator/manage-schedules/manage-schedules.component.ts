  ...
  <div class="min-h-screen bg-slate-50 dark:bg-slate-900">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-5">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div class="p-6 space-y-4">
              <p class="text-sm text-slate-600 dark:text-slate-300">
                ...
                <span class="font-bold text-slate-900 dark:text-white">{{ cancelTarget()!.busCode }}</span>
                on route <span class="font-mono font-semibold text-slate-700 dark:text-slate-200">{{ cancelTarget()!.routeCode }}</span>.
                All passengers will be notified and refunded.
              </p>
              <div>
      ...
      <div class="flex gap-3">
        <input [(ngModel)]="search" placeholder="Search bus or route..." class="flex-1 min-w-[200px] text-sm border border-slate-200 dark:border-slate-600 rounded-xl px-3 py-2 bg-white dark:bg-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500"/>
      </div>
                  ...
                  <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors" [class.opacity-60]="s.isCancelledByOperator">
                    ...
                    <td class="px-6 py-4 text-right">
                      <div class="flex items-center justify-end gap-2">
                        @if (!s.isCancelledByOperator) {
                          <button (click)="openEdit(s)" class="px-3 py-1.5 text-xs font-semibold rounded-lg border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">Edit</button>
                          <button (click)="openCancelTrip(s)" class="px-3 py-1.5 text-xs font-semibold rounded-lg border border-orange-200 dark:border-orange-700 text-orange-600 dark:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors">Cancel Trip</button>
                        }
                        <button (click)="remove(s.id)" class="px-3 py-1.5 text-xs font-semibold rounded-lg border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">Delete</button>
                      </div>
                    </td>
                  </tr>
                }
...
export class ManageSchedulesComponent implements OnInit {
  ...
  confirmCancel() {
    ...
    this.scheduleSvc.delete(target.id).subscribe({
      next: () => {
        this.schedules.update(l => l.map(s => s.id === target.id ? { ...s, isCancelledByOperator: true } : s));
        this.cancelling.set(false);
        this.cancelTarget.set(null);
        this.toast.success('Trip cancelled. Passengers will be notified.');
      },
      error: err => {
        this.cancelling.set(false);
