    ...
    <div class="bg-white border-b border-slate-200 shadow-sm">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-base font-bold text-slate-900">Operator Performance</h1>
            <p class="text-xs text-slate-500">Platform-wide operator metrics</p>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <input [(ngModel)]="search" placeholder="Search operator…"
            class="text-sm border border-slate-200 rounded-xl px-3 py-1.5 bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-red-500 w-48"/>
        </div>
      </div>
    </div>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            ...
            <p class="text-2xl font-extrabold text-slate-900">{{ operators().length }}</p>
            <p class="text-xs text-slate-500 mt-1">Total Operators</p>
          </div>
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <div class="w-11 h-11 rounded-xl bg-yellow-50 flex items-center justify-center text-xl mb-3">💰</div>
            <p class="text-2xl font-extrabold text-slate-900">₹{{ platformRevenue() | number:'1.0-0' }}</p>
            <p class="text-xs text-slate-500 mt-1">Platform Revenue</p>
          </div>
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <div class="w-11 h-11 rounded-xl bg-purple-50 flex items-center justify-center text-xl mb-3">🎫</div>
            <p class="text-2xl font-extrabold text-slate-900">{{ platformBookings() }}</p>
          ...
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            ...
            <p class="text-2xl font-extrabold text-slate-900">{{ platformAvgRating() | number:'1.1-1' }}</p>
            <p class="text-xs text-slate-500 mt-1">Avg Platform Rating</p>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h2 class="font-bold text-slate-900">Operator Leaderboard</h2>
            <span class="text-xs text-slate-400">{{ filtered().length }} operators</span>
          </div>

          @if (filtered().length === 0) {
            <div class="text-center py-16 text-slate-400">
              <p class="text-4xl mb-3">🔍</p>
              <p class="text-sm">No operators found</p>
            </div>
          } @else {
              ...
              <table class="w-full text-sm">
                <thead>
                  <tr class="bg-slate-50 border-b border-slate-100">
                    ...
                    <th class="text-left px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Operator</th>
                    <th class="text-right px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Buses</th>
                    <th class="text-right px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Bookings</th>
                    <th class="text-right px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Revenue</th>
                    <th class="text-right px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Rating</th>
                    <th class="text-right px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Cancel Rate</th>
                    <th class="text-right px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Score</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                  @for (op of filtered(); track op.operatorId; let i = $index) {
                    <tr class="hover:bg-slate-50 transition-colors">
                      ...
                      <td class="px-6 py-4">
                        <p class="font-semibold text-slate-800">{{ op.companyName }}</p>
                        <p class="text-xs text-slate-400">{{ op.ownerName }}</p>
                      </td>
                      <td class="px-4 py-4 text-right font-medium text-slate-700">{{ op.totalBuses }}</td>
                      <td class="px-4 py-4 text-right">
                        <p class="font-medium text-slate-700">{{ op.totalBookings }}</p>
                      ...
                      <td class="px-4 py-4 text-right">
                        <div class="flex items-center justify-end gap-1">
                          ...
                          <span class="font-semibold text-slate-800">{{ op.averageRating | number:'1.1-1' }}</span>
                          <span class="text-xs text-slate-400">({{ op.totalReviews }})</span>
                        </div>
                      </td>
                      <td class="px-4 py-4 text-right">
                      ...
                      <td class="px-4 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                          ...
                          <span class="text-xs font-bold text-slate-700">{{ performanceScore(op) | number:'1.0-0' }}</span>
                        </div>
                      </td>
                    </tr>
                  }
