    ...
    <div class="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <div class="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shadow-sm">
        <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h1 class="text-base font-bold text-slate-900 dark:text-white">Stops & Cities</h1>
        ...
        @if (!selectedCity) {
          @if (loadingCities()) {
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center py-20 text-center">
              ...
              <p class="font-semibold text-slate-700 dark:text-white">No cities found</p>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              @for (city of filteredCities(); track city.city) {
                <button (click)="selectCity(city.city)"
                  class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 text-left hover:border-red-300 hover:shadow-md transition-all group">
                  <div class="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center mb-3 group-hover:bg-red-50 transition-colors">
                  ...
                  <p class="font-semibold text-slate-900 dark:text-white group-hover:text-red-700 transition-colors text-sm">{{ city.city }}</p>
                  <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{{ city.stopCount }} stop{{ city.stopCount !== 1 ? 's' : '' }}</p>
                </button>
              }
            </div>
          }
        ...
        @if (selectedCity) {
          @if (loadingStops()) {
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center py-20 text-center">
              ...
              <p class="font-semibold text-slate-700 dark:text-white">No stops in {{ selectedCity }}</p>
              <p class="text-sm text-slate-400 dark:text-slate-500 mt-1">Add the first stop for this city</p>
              <button (click)="openAddStop()" class="mt-4 px-5 py-2.5 bg-red-600 text-white text-sm font-semibold rounded-xl hover:bg-red-700 transition-colors">
                Add Stop
              </button>
            </div>
          } @else {
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
              ...
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-slate-100 dark:border-slate-700 text-left bg-slate-50 dark:bg-slate-700/50">
                    <th class="px-6 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Stop Name</th>
                    <th class="px-6 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide hidden md:table-cell">City</th>
                    <th class="px-6 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide hidden lg:table-cell">Coordinates</th>
                    ...
                    <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-700/30 transition-colors">
                      <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                          ...
                          <span class="font-medium text-slate-900 dark:text-white">{{ stop.name }}</span>
                        </div>
                      </td>
                      <td class="px-6 py-4 text-slate-500 dark:text-slate-400 hidden md:table-cell">{{ stop.city }}</td>
                      <td class="px-6 py-4 hidden lg:table-cell">
                        @if (stop.latitude && stop.longitude) {
                          <span class="font-mono text-xs text-slate-400 dark:text-slate-500">{{ stop.latitude.toFixed(4) }}, {{ stop.longitude.toFixed(4) }}</span>
                        } @else {
                      ...
                      <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                          <button (click)="openEditStop(stop)"
                            class="px-3 py-1.5 text-xs font-medium border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                            Edit
                          </button>
                          <button (click)="deleteStop(stop)"
                            class="px-3 py-1.5 text-xs font-medium border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 rounded-xl hover:bg-red-50 transition-colors">
                            Delete
                          </button>
                        </div>
                      </td>
        ...
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-700">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
              <h2 class="font-semibold text-slate-900 dark:text-white">Add New Stop</h2>
              <button (click)="closeAddStop()" class="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors text-slate-400">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-4">
              <div>
                <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide mb-1.5">Stop Name *</label>
              ...
              <div>
                <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide mb-1.5">City *</label>
                <input [(ngModel)]="newStop.city" placeholder="e.g. Mumbai"
                  class="w-full px-3 py-2.5 text-sm border border-slate-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-400 transition-colors"/>
              </div>
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide mb-1.5">Latitude <span class="text-slate-400 font-normal normal-case">(optional)</span></label>
                ...
                <div>
                  <label class="block text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide mb-1.5">Longitude <span class="text-slate-400 font-normal normal-case">(optional)</span></label>
                  <input type="number" step="0.0001"
              ...
              <div class="flex gap-3 pt-2">
                <button (click)="closeAddStop()"
                  class="flex-1 py-2.5 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  Cancel
                </button>
      ...
      @if (showEditStop() && editStopData()) {
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-700">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
              <h2 class="font-semibold text-slate-900 dark:text-white">Edit Stop</h2>
              <button (click)="closeEditStop()" class="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors text-slate-400">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                </svg>
              </button>
