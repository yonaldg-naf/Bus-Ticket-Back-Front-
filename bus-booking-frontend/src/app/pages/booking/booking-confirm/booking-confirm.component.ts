    ...
    <div class="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shadow-sm">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4">
        ...
        <div class="flex items-center gap-2 mt-3">
          ...
          <div class="step-done"><span class="step-dot-done">✓</span><span class="hidden sm:inline">Passenger Details</span></div>
          <div class="flex-1 h-0.5 bg-red-300 rounded"></div>
          <div class="step-active"><span class="step-dot-active">3</span><span class="hidden sm:inline">Payment</span></div>
        </div>
      </div>
    </div>
                ...
                @for (p of booking()!.passengers; track p.seatNo) {
                  <div class="px-5 py-3.5 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                      <div class="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center text-red-700 font-bold text-sm">{{ p.name[0].toUpperCase() }}</div>
                      <div>
          ...
          <div class="space-y-4">
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-5 sticky top-20">
              ...
              @if (booking()!.status === BookingStatus.Pending) {
                <button (click)="processPayment()" [disabled]="payLoading()" class="btn-primary w-full py-3.5 text-base mb-2">
                  @if (payLoading()) {
                ...
                <div class="border border-slate-200 dark:border-slate-600 rounded-xl p-3 mb-3">
                  ...
                  <button (click)="processWalletPayment()" [disabled]="payLoading() || walletBalance() < finalAmount()"
                    ...
                    [class]="walletBalance() >= finalAmount() ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-400 cursor-not-allowed'">
...
export class BookingConfirmComponent implements OnInit {
  ...
  downloadTicket() {
    ...
    const names = b.passengers.map(p =>
      `<tr><td style="padding:6px 12px;border-bottom:1px solid #f1f5f9">${p.name}</td><td style="padding:6px 12px;border-bottom:1px solid #f1f5f9">${p.age ?? '—'}</td><td style="padding:6px 12px;border-bottom:1px solid #f1f5f9;font-weight:700">${p.seatNo}</td></tr>`
    ).join('');
    ...
    th{background:#f8fafc;padding:8px 12px;text-align:left;font-size:11px;text-transform:uppercase;color:#64748b}
    ...
    <div class="ticket">
      <div class="header"><h1>🚌 SwiftRoute</h1><p>E-Ticket · Booking #${b.id.slice(0, 8).toUpperCase()}</p></div>
      <div class="body">
        ...
        <h3 style="margin:20px 0 8px;font-size:13px;color:#64748b;text-transform:uppercase">Passengers</h3>
  ...
  processPayment() {
    ...
    this.bookingSvc.pay(this.booking()!.id, {
      ...
      error: (err: any) => { this.payLoading.set(false); this.toast.error(err.error?.message ?? 'Payment failed.'); },
    });
  }
  ...
  cancelBooking() {
    ...
    this.bookingSvc.cancelPost(this.booking()!.id).subscribe({
      ...
      error: (err: any) => this.toast.error(err.error?.message ?? 'Cancellation failed.'),
    });
  }
}
